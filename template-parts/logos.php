...
<section x-data="{viewCount: 4}" class="container bg-primary-100 py-8 mt-16">
    ...
    <div class="grid lg:grid-cols-4 gap-8 mt-8">
        <?php foreach($data['client_logos'] as $index => $logo): ?>
            <div 
                ...
                <img class="w-auto h-auto max-h-full max-w-full" src="<?php echo esc_url($logo['image']['url']); ?>" alt="<?php echo esc_attr(get_post_meta($logo['image']['id'], '_wp_attachment_image_alt', true)); ?>" loading="lazy" />
            </div>
        <?php endforeach; ?>
    </div>
    ...
    <div class="flex justify-center items-center mt-8">
        <button 
            ...
            View All
            <span class="w-5 h-5 inline-flex justify-center items-center"><?php echo valtes_svg('chevron-down'); ?></span>
        </button>
    </div>
</section>
...
async function main() {
  try {
    ...
    const currentFunctionsVersion = (await fs.readFile(functionsFilePath, 'utf8')).match(/define\('PIVOTAL_ACCESSIBILITY_VERSION',\s*'(\d+\.\d+\.\d+)'\);/)[1];
    ...
    const updatedFunctionsData = functionsFileData.replace(/(define\('PIVOTAL_ACCESSIBILITY_VERSION',\s*)'(\d+\.\d+\.\d+)'(\);)/, `$1'${newFunctionsVersion}'$3`);
